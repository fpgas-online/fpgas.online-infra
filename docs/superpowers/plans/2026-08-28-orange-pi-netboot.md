...
```yaml
    ...
    - name: Check FEL-boot units on the hub host
      command: systemctl list-units --all --no-legend --plain 'fpgas-felboot@*'
      changed_when: false
    ...
    - name: Assert every attached board was FEL-booted from this host
      assert:
        that: >-
          ...
          | map(attribute='usb') | map('regex_replace', '^', 'fpgas-felboot@')
          | map('regex_replace', '$', '.service') | list
          | difference(verify_pi_felboot.stdout_lines | map('regex_search', 'fpgas-felboot@[^ ]+') | list) | length == 0
        fail_msg: "Missing fpgas-felboot@ instances on {{ ansible_hostname }}: {{ verify_pi_felboot.stdout }}"
